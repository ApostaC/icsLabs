...
do
    echo "Fetching ./tiny/${file} into ${PROXY_DIR} using the proxy"
	echo "Command is $NOPROXY_DIR ${file} \"http://localhost:${tiny_port}/${file}\" \"http://localhost:${proxy_port}\""
    download_proxy $PROXY_DIR ${file} "http://localhost:${tiny_port}/${file}" "http://localhost:${proxy_port}"
done
...
echo "Command is $NOPROXY_DIR ${FETCH_FILE} \"http://localhost:${tiny_port}/${FETCH_FILE}\" \"http://localhost:${proxy_port}\""
